...
@section ('title', trans('category::labels.backend.categories.management') . ' | ' . trans('category::labels.backend.categories.create'))

@section('page-header')
    <h1>
        {{ trans('category::labels.backend.categories.management') }}
        <small>{{ trans('category::labels.backend.categories.create') }}</small>
    </h1>
@endsection

@section('content')
    {{ Form::open(['route' => 'admin.category.store', 'class' => 'form-horizontal', 'files' => true ,'role' => 'form', 'method' => 'post', 'id' => 'create-category']) }}

        <div class="box box-success">
            <div class="box-header with-border">
                <h3 class="box-title">{{ trans('category::labels.backend.categories.create') }}</h3>
