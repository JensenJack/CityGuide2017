...
class UserRepository extends Repository
{
    ...
    public function create($input)
    {
        ...
        if(isset($data['is_member'])){
             $this->createMemberStub($data);
        }
        if(isset($data['is_supplier'])){
             $this->createSupplierStub($data);
        }
    ...
    public function updateMember(Model $member, array $input)
    {
        $data = $input['data'];

        DB::transaction(function () use ($member, $data) {
            if (parent::update($member, $data)) {
                //For whatever reason this just wont work in the above call, so a second is needed for now
                $member->status = isset($data['status']) ? 1 : 0;
                $member->confirmed = isset($data['confirmed']) ? 1 : 0;
                parent::save($member);
    ...
    public function updateSupplier(Model $supplier, array $input)
    {
        $data = $input['data'];

        DB::transaction(function () use ($supplier, $data) {
            if (parent::update($supplier, $data)) {
                //For whatever reason this just wont work in the above call, so a second is needed for now
                $supplier->status = isset($data['status']) ? 1 : 0;
                $supplier->confirmed = isset($data['confirmed']) ? 1 : 0;
                parent::save($supplier);
    ...
    public function createUserStub($input)
    {
        ...
        $user->is_member = isset($input['is_member']) ? 1 : 0;
        $user->is_supplier = isset($input['is_supplier']) ? 1 : 0;
    ...
    public function createMemberStub($input)
    {
    ...
    public function createSupplierStub($input)
    {
    ...
    public function getmemberForDataTable()
    {
       
     return $this->query()->where('is_member',1)->get();
       
    }

     public function getsupplierForDataTable()
    {
       
     return $this->query()->where('is_supplier',1)->get();
